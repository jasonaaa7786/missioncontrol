...
async function writeHeartbeatFile(agentId, message) {
  // Write heartbeat message to agent's HEARTBEAT.md file
  const heartbeatPath = path.join(process.env.HOME, 'livescape-marketing', agentId, 'HEARTBEAT.md');
  
  const content = `# Heartbeat Check - ${new Date().toISOString()}

${message}

---

*This file is auto-generated by the heartbeat scheduler. Delete after reading.*
`;
  
  fs.writeFileSync(heartbeatPath, content, 'utf8');
  console.log(`[${agentId}] Wrote HEARTBEAT.md`);
...
async function runAgentHeartbeat(agentId, reason, tasks = []) {
  ...
  try {
    ...
    if (tasks.length > 0) {
      heartbeatMessage += `You have **${tasks.length} active task(s)** assigned to you:\n\n`;
      tasks.forEach((task, i) => {
        heartbeatMessage += `### ${i + 1}. [${task.status.toUpperCase()}] ${task.title}\n\n`;
        if (task.description) {
          heartbeatMessage += `${task.description}\n\n`;
        }
        heartbeatMessage += `- **Priority:** ${task.priority}\n`;
        heartbeatMessage += `- **Task ID:** ${task.id}\n\n`;
      });
      heartbeatMessage += `\n**Instructions:**\n`;
      heartbeatMessage += `1. Read each task carefully\n`;
      heartbeatMessage += `2. Work on the highest priority tasks first\n`;
      heartbeatMessage += `3. Report progress by adding comments to tasks (if integrated)\n`;
      heartbeatMessage += `4. Update task status when appropriate\n\n`;
    } else {
      heartbeatMessage += `No active tasks assigned. Check for:\n\n`;
      heartbeatMessage += `- New opportunities in your domain\n`;
      heartbeatMessage += `- Proactive research needs\n`;
      heartbeatMessage += `- Updates to existing intelligence\n\n`;
    }
    
    // Write to agent's HEARTBEAT.md file
    const heartbeatPath = await writeHeartbeatFile(agentId, heartbeatMessage);
    
    // Run agent using OpenClaw with the livescape profile
    // The agent will read HEARTBEAT.md automatically if configured
    const agentDir = path.join(process.env.HOME, 'livescape-marketing', agentId);
    
    // Simple approach: Just trigger the agent to wake up and check HEARTBEAT.md
    // Using a simple file-based trigger that the agent can detect
    const command = `cd ${agentDir} && echo "Heartbeat triggered at $(date)" >> .heartbeat-log`;
    
    console.log(`[${agentId}] Triggering heartbeat check...`);
    await execAsync(command, { timeout: 5000 });
    
    console.log(`[${agentId}] ✅ Heartbeat triggered (agent should check HEARTBEAT.md on next run)`);
    ...
    await prisma.activity.create({
      data: {
        ...
        agentId: agentId,
        taskId: tasks[0]?.id || null,
        message: `${agentId} heartbeat triggered - HEARTBEAT.md updated`,
    ...
    // Update task timestamps to show activity
    if (tasks.length > 0) {
    ...
    return { success: true, heartbeatPath, taskCount: tasks.length };
...
async function runHeartbeatCycle() {
  ...
  console.log(`   - Failed: ${results.filter(r => r.error && !r.skipped).length}`);
  
  const triggered = results.filter(r => r.success);
  if (triggered.length > 0) {
    console.log(`\n   Triggered agents:`);
    triggered.forEach(r => {
      console.log(`   - ${r.agentId}: ${r.taskCount} tasks`);
    });
  }
